...
$app = @(
	...
	("Google Chrome",
	 ...
	 ("Mozilla Firefox",
	  ...
	  "Installation package\Browser",
	  "",
	  "http://ftp.mozilla.org/pub/firefox/releases/98.0/win32/en-US/Firefox%20Setup%2098.0.exe",
	  "http://ftp.mozilla.org/pub/firefox/releases/98.0/win64/en-US/Firefox%20Setup%2098.0.exe",
	  "",
	  "",
	  "Firefox*",
	  "/s",
		...
		zip
		{
			Switch ($act)
			{
				...
				To
				{
					...
					if (Test-Path $OutAny) {
						Write-Host "    - Unzip only".PadRight(28) -NoNewline
